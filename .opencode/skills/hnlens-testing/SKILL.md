...
| `a11ytest.mjs` | `test:a11y` | **Accessibility (axe-core, WCAG 2 A/AA)** on Home, Item permalink, Settings, and the open comments drawer. **Fails on any serious/critical** issue; reports moderate/minor as warnings. Hermetic. (Caught real bugs: `aria-hidden-focus` on the closed drawer → `inert`; low-contrast tokens → darkened.) |
...
| `stress.mjs` | `test:stress` | Adversarial: rapid tab/save/hide/drawer, nasty search (XSS/long/unicode), invalid routes, extreme weights, weird HN usernames, clear-data-mid-use. Watches for ErrorBoundary crashes + console errors. (Informational; the only expected finding is a benign 404 network log for a genuinely nonexistent item id.) |
...
4. Drive the **real UI over a mocked HN API** (`page.route` on `hacker-news.firebaseio.com`) so the
   ...
   opens with the full score→weights→data trace. NOTE: the app mounts a comments drawer with
   `role="dialog"` too — select the explainer by content (`/How the score adds up/`), not the first
   `[role=dialog]`.
