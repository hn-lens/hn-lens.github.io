# Hacker Lens

A smarter, private Hacker News reader that runs entirely in your browser. It takes the
original Hacker News data and adds a personalized recommendation feed, custom ranking, a
better reading UI, and **local** AI (embeddings + an in-browser LLM) for summaries and
recommendations — with **no backend**. All your activity and models stay in your browser
(IndexedDB + localStorage), and it deploys as a static site to GitHub Pages.

## Features

- **For You feed** — re-ranks stories from your reading activity, follows, keyword
  interests, embedding similarity, and a locally-trained logistic reranker. Every card
  shows *why* it was recommended. One toggle away from the **original HN ordering**
  (Top / New / Best / Ask / Show / Jobs).
- **Local models, pluggable** — swap between:
  - **Embeddings** (Transformers.js, WebGPU/WASM) for similarity ranking & "more like this".
  - **LLM** (WebLLM, WebGPU) for story TL;DRs and discussion summaries.
  - **Learned reranker** — a logistic-regression model trained in-browser on your clicks,
    saves, and hides. The model lives behind one small surface (`src/lib/ranking/logistic.ts` for
    training/prediction, `strategies.ts` for how its output is blended), so swapping in a different
    learner means touching those two files.
- **Great comments UX** — full nested threads fetched in one request (Algolia), collapsible
  subtrees, "new since last visit" highlighting, sort by default/newest/oldest/most-replies, AI thread
  summaries — on a full discussion page (`/item/:id`). Feed cards can also show the single
  standout comment inline (a **"Top comments"** toggle in the feed header) so you can read the
  best take without opening the thread.
- **Full-text search** across all of HN (Algolia).
- **Local-first interactions** — save, mark "not interested" (a downvote that trains the
  ranker), follow/mute domains & users, tunable ranking weights, dark/light themes. Real
  upvote/comment deep-link out to Hacker News.
- **Works offline** — a service worker precaches the app; your Saved and Read stories stay
  readable with no connection. The app tells you when you're offline, recovers automatically on
  reconnect, and can be installed for one-tap offline access.

## Data sources (both CORS-enabled, no key, browser-only)

- **Firebase API** (`hacker-news.firebaseio.com`) — live front-page/new/best/ask/show/job
  ordering + item and user data.
- **Algolia HN Search API** (`hn.algolia.com/api/v1`) — search, filtering, and whole nested
  comment trees in a single request.

Note: writes (upvote/comment/favorite) can't be done from a static app because
`news.ycombinator.com` has no CORS — those actions deep-link to HN.

A few further network calls exist and are fully disclosed in
[SECURITY.md](SECURITY.md#what-data-leaves-your-browser). **On by default** (toggleable
in Settings → Privacy): story **favicons** load from Google's favicon service (turn it off for
letter monograms only). Two **opt-in, off-by-default** features send more data only when *you*
enable them — **linked-article text** (via a public reader-proxy chain, powering the inline
Article view, read-aloud, AI summaries, and ranking) and **BYO cloud-LLM summaries** (your own
API key, stored only in your browser). One more is worth knowing about: **read-aloud** uses the
browser's built-in `speechSynthesis`, and on several platforms the default system voice is a
*network* voice — so the text being read can leave the device via the OS, not via this app. Which
voice is used is an OS setting, outside Hacker Lens's control. Everything else stays on your
device; [SECURITY.md](./SECURITY.md) lists every call in full.

## Tech stack

React + TypeScript + Vite + Tailwind CSS · TanStack Query · Zustand · Dexie (IndexedDB) ·
Transformers.js · WebLLM. Heavy ML libraries are code-split and loaded only when enabled.

## Develop

```bash
npm install
npm run dev      # http://localhost:5173
npm run build    # production build to dist/
npm run preview  # preview the production build
```

Requires a browser with **WebGPU** (recent Chrome/Edge) to run the local LLM; embeddings
fall back to WASM without it.

## Deploy (GitHub Pages)

Pushing to `main` first runs `.github/workflows/ci.yml` — the `npm run verify` gate (typecheck,
lint, build, and the test harnesses). Only when CI succeeds does `.github/workflows/deploy.yml`
run (via `workflow_run`), building and publishing `dist/` to GitHub Pages; a failing gate blocks
the deploy. In the repo settings, set **Pages → Build and deployment → Source: GitHub Actions**.
The app uses a hash router and relative asset base, so it works on any Pages URL (user or project
site) without extra configuration.

## Contributing

Contributions are welcome — see [CONTRIBUTING.md](./CONTRIBUTING.md) for setup, the test
suite, and coding guidelines. For security reports, see [SECURITY.md](./SECURITY.md).

## License

[MIT](./LICENSE) © Jianling Zhong

Bundled third-party dependencies and their licenses (including the Apache-2.0 notices this
project is required to propagate) are listed in
[THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md), generated by `node scripts/gen-notices.mjs`.
That list is the full transitive runtime closure — everything that can reach a user's bundle — not
just the dependencies named in `package.json`.

**Built with Llama.** The optional on-device summarizer uses Meta's
[Llama 3.2](https://github.com/meta-llama/llama-models/blob/main/models/llama3_2/LICENSE), under the
Llama 3.2 Community License. This project does not redistribute the weights — your browser downloads
them from the model host if you turn on-device AI on.
