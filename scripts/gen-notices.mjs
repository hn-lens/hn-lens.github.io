 ...
 *
 * The output is a function of the attribution-relevant facts ONLY — the set of packages, each one's
 * license and homepage, the upstream NOTICE text, and the model catalog. Package versions are
 * excluded so that `--check` can stay byte-exact: it fails when an attribution fact moves, and a
 * version bump on its own is not one. Adding a version anywhere below makes every dependency bump a
 * gate failure that only a human with a checkout can clear. `scripts/noticesbumptest.mjs` holds
 * that line.
 *
...
lines.push('');
lines.push('Package versions are not listed here. What attribution attaches to is which packages are');
lines.push('redistributed, under which license, and the NOTICE text they carry; the exact version of each');
lines.push('is recorded in `package-lock.json`, which is committed alongside this file.');
lines.push('');
...
if (apache.length) {
  ...
  lines.push('');
  for (const r of apache) lines.push(`- ${r.name}`);
  lines.push('');
...
if (dual.length) {
  ...
  for (const r of dual) {
    ...
    lines.push(`- **${r.name}** — offered as \`${r.license}\`. Hacker Lens elects the`);
